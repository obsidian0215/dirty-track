...
- Results are written to the `results/` directory by `mig-scripts/result_writer.py` in TSV format.
- A helper to convert those TSV files to CSV is provided: `mig-scripts/tsv_to_csv.py`.

Quick note for TSV -> CSV conversion
----------------------------------

To convert all TSV result files into CSV files next to the originals:
