...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timeout", "--redis-timeout", type=int, default=4000, help="Redis timeout in milliseconds for YCSB."
    )
    parser.add_argument(
...
def run_ycsb_cmd(cmd, ignore_error=False, background=False):
    # 在第三台机器上执行YCSB命令
    # 如果 background=True，则在目标机器上使用 nohup 和 & 将进程放入后台
    if background:
        remote_cmd = f"nohup {cmd}  &"
    else:
        remote_cmd = cmd

    # Use argument list for ssh to avoid local shell=True and keep quoting predictable.
    res = run_remote_cmd(remote_cmd, target_ip=YCSB_IP, ignore_error=ignore_error, background=background, quiet=True)
...
def destination_prepare():
    ...
    recvtty_cmd = (
        "PATH=$PATH:/root/go/bin recvtty -m null "
        f"/runc/containers/{container_name}/console.sock > /tmp/recvtty_debug.log 2>&1 "
    ...
    ts = int(time.time())
    script_name = globals().get('DEST_SCRIPT', 'destination.py').replace('.', '_')
...
def configure_network_do(interface, rules, is_remote=False, target_ip=None, ignore_error=False):
    ...
    if is_remote and not target_ip:
        msg = "configure_network_do requires target_ip when is_remote=True"
