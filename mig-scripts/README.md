...
- Results are written via `result_writer` when orchestrators can extract stats lines from source output; CSV is the default format.
- If you need the project-wide README updated instead, tell me and I'll add a short note there as well.

## Legacy TSV -> CSV conversion tool

A small utility `tsv_to_csv.py` is included to safely convert legacy TSV result files into CSV. Features:
...
```bash
# Convert legacy TSVs in results/ to CSVs next to the originals
...
The tool is conservative about tabs inside fields: it treats tabs as column separators and does not attempt to unescape them. If your legacy `results/*.tsv` contains embedded tabs inside a logical field, consider post-processing those fields to remove or replace internal tabs before conversion.
# mig-scripts
