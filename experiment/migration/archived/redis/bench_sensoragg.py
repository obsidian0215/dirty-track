...
def main():
    ...
    # 负载参数
    parser.add_argument("--threads", "--concurrency", dest="threads", default=4, type=int,
    ...
    # dataset: if not provided, resolved post-parse to repo datasets/
    parser.add_argument("--dataset", default=None,
                        help="Path to dataset directory (default: repo datasets/)")
    ...
    parser.add_argument("--rps", "--qps", "--max-requests-per-second", dest="rps",
                        type=int, default=0, help="Maximum requests per second (default: no limit)")
    ...
    if bench_common:
        ...
        # fallback to repo datasets path (non-fatal) if not provided
        if getattr(args, 'dataset', None) is None:
