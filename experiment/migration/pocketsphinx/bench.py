#!/usr/bin/env python3
"""
Pocketsphinx bench supporting --duration / --threads and dataset resolution.
"""
import argparse
import csv
import os
...
if bench_common:
    ...
    parser.add_argument('--threads', '--concurrency', dest='threads', type=int, default=2)
    parser.add_argument('--rps', '--qps', dest='rps', type=int, default=0)
parser.add_argument('--url', required=False, help='Base URL e.g. http://127.0.0.1:8080/transcribe')
...
parser.add_argument('--files', default=None, help='Comma-separated list of audio files to cycle through (relative to --dataset if not absolute)')
if not bench_common:
...
if args.files:
    ...
    if not file_paths:
        raise SystemExit('no audio files found in dataset dir: %s' % ds)
...
def do_one_request():
    ...
    try:
        ...
        r = sess.post(server, files={'file': ('audio.wav', payload)}, timeout=60)
