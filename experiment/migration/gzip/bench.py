...
parser.add_argument('--files', default=None, help='Comma-separated list of files to cycle through (relative to --dataset if not absolute)')
...
if bench_common:
    ...
    try:
        if args.dataset == bench_common.DEFAULT_DATASET_DIR or not args.dataset:
            args.dataset = '/runc/datasets/compress'
    except Exception:
        if not args.dataset:
            args.dataset = '/runc/datasets/compress'
else:
...
def worker_duration(url, end_time):
    ...
    while time.time() < end_time:
        ...
        start = time.time()
        try:
            payload = get_next_data()
            r = sess.post(args.url, data=payload, headers={'Content-Type': 'application/octet-stream'}, timeout=60)
...
def worker_iters(url, idx):
    ...
    try:
        payload = get_next_data()
        r = requests.post(args.url, data=payload, headers={'Content-Type': 'application/octet-stream'}, timeout=60)
        status = r.status_code
        preview = r.text[:200]
    except Exception as e:
        status = None
